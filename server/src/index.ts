...
//     await db.query(
//       "INSERT INTO user_stats (user_id, postCount, commentCount, likeCount, friendCount, shareCount) VALUES (?, 0, 0, 0, 0, 0)",
//       [userId]
//     );
...
// Also store the username and user_id in the response for a later token
...
//       "SELECT user_id, username, name, profile_picture_url FROM users WHERE username = ? AND password_hash = ?",
//       [username, password]
//     );
...
//   const { user_id, name, location, gender, age, interests, bio, school } =
//     req.body;
...
//       "SELECT user_id, name, email, location, gender, age, interests, bio, school, cover_picture, profile_picture_url FROM users WHERE user_id = ?",
//       [userId]
//     );
